...
int formSDcut(probType *prob, cellType *cell, vector X, int lenX, double lb, int iter) {
    oneCut  *cut;

    cut = newCut(lenX, cell->omega->cnt, iter);

    /* compute the candidate cut */
    //if ( SDCut(prob->num, prob->coord, cell->sigma, cell->delta, cell->omega, cut, X, cell->k, &cell->dualStableFlag, cell->pi_ratio, lb) ) {
    if ( SDCut(prob->num, prob->coord, cell->sigma, cell->delta, cell->omega, cut, X, iter, &cell->dualStableFlag, cell->pi_ratio, lb) ) {
        errMsg("algorithm", "compute cut", "failed to create a new SD cut", 0);
        return -1;
    }

    /* The cut agent cellType is not ready with quadratic updates */
    cell->cuts->val[cell->cuts->cnt] = cut;

#ifdef CUT_CHECK
    printf("\tAgent-%0.2d cut: alpha = %.2lf, beta = ", cell->ID, cut->alpha);
...
int SDCut(numType *num, coordType *coord, sigmaType *sigma, deltaType *delta, omegaType *omega, oneCut *cut, vector Xvect, int numSamples,
...
iType computeIstar(numType *num, coordType *coord, sigmaType *sigma, deltaType *delta, vector Xvect, vector PiCbarX, int obs, int ictr, BOOL pi_eval, double *argmax) {
    iType 	ans;
    ans.delta = 0;
    ans.sigma = 0;
    double 	arg;
...
iType compute_new_istar(int obs, oneCut *cut, sigmaType *sigma, deltaType *delta, vector Xvect, numType *num, coordType *coord, vector PiCbarX, double *argmax, int ictr) {
    double arg;
    int sig_pi, del_pi;
    int new_pisz;
    iType ans;
...
int reduceCuts(cellType **cell, vector candidX, vector pi, int lbType, int betaLen, double lb, int agentIdx, LPptr lp) {
    ...
    minObs 	  = cell[0]->k;
    oldestCut = cell[agentIdx]->cuts->cnt;

    /* identify the oldest loose cut */
    for (idx = 0; idx < cell[agentIdx]->cuts->cnt; idx++) {
        if ( idx == cell[agentIdx]->iCutIdx && cell[agentIdx]->cuts->val[idx]->rowNum < 0)
            /* avoid dropping incumbent cut and newly added cuts */
            continue;

        if (cell[agentIdx]->cuts->val[idx]->cutObs < minObs && DBL_ABS(pi[cell[agentIdx]->cuts->val[idx]->rowNum + 1]) <= config.TOLERANCE ) {
            minObs = cell[agentIdx]->cuts->val[idx]->cutObs;
            oldestCut = idx;
        }
    }

    /* if the oldest loose cut is the most recently added cut, then the cut with minimium cut height will be dropped */
    if ( oldestCut == cell[agentIdx]->cuts->cnt ) {
        //MARK:cell[agentIdx]->k to cell[0]->k
        //minHeight = cutHeight(lbType, cell[agentIdx]->cuts->val[0], cell[agentIdx]->k, candidX, betaLen, lb);
        minHeight = cutHeight(lbType, cell[agentIdx]->cuts->val[0], cell[0]->k, candidX, betaLen, lb);
        oldestCut = 0;

        for (idx = 1; idx < cell[agentIdx]->cuts->cnt; idx++) {
            if (idx == cell[agentIdx]->iCutIdx)
                continue;

            //MARK:cell[agentIdx]->k to cell[0]->k
            //height = cutHeight(lbType, cell[agentIdx]->cuts->val[idx], cell[agentIdx]->k, candidX, betaLen, lb);
            height = cutHeight(lbType, cell[agentIdx]->cuts->val[idx], cell[0]->k, candidX, betaLen, lb);
            if (height < minHeight) {
                minHeight = height;
                oldestCut = idx;
            }
    ...
    status = dropCut(cell, oldestCut, agentIdx, lp);
    if ( status ) {
        errMsg("algorithm", "reduceCuts", "failed to drop a cut", 0);
        return -1;
    }

    /* candidate cut is moved to the position of the removed cut. Therefore, update the cCutIdx */
    cell[agentIdx]->cCutIdx = oldestCut;
...
int dropCut(cellType **cell, int cutIdx, int agentIdx, LPptr lp) {
    int idx, status, deletedRow, i;

    deletedRow = cell[agentIdx]->cuts->val[cutIdx]->rowNum;
    /* Get rid of the indexed cut on the solver */
    status = removeRow(lp, deletedRow, deletedRow);
    if ( status ) {
        printf("stopped at %d",cell[0]->k);
        errMsg("solver", "dropCut", "failed to remove a row from master problem", 0);
        return 1;
    }
    freeCut(cell[agentIdx]->cuts->val[cutIdx]);

    /* move the last cut to the deleted cut's position (structure) */
    cell[agentIdx]->cuts->val[cutIdx] = cell[agentIdx]->cuts->val[--cell[agentIdx]->cuts->cnt];

    /* if the swapped cut happens to be the incumbent cut, then update its index */
    if ( cell[agentIdx]->iCutIdx == cell[agentIdx]->cuts->cnt )
        cell[agentIdx]->iCutIdx = cutIdx;

    if ( config.MULTI_CUT ) {
    	for (i = 1; i < numAgents; i++) {
    		for (idx = 0; idx < cell[i]->cuts->cnt; idx++) {
    			if (cell[i]->cuts->val[idx]->rowNum > deletedRow)
    				--cell[i]->cuts->val[idx]->rowNum;
    		}
    	}
    }
    else {
		for (idx = 0; idx < cell[0]->cuts->cnt; idx++) {
			if (cell[0]->cuts->val[idx]->rowNum > deletedRow)
				--cell[0]->cuts->val[idx]->rowNum;
		}
		/* remove the agent cuts that were used to create the dropped cuts. */
		for ( i = 1; i < numAgents; i++ ) {
			freeCut(cell[i]->cuts->val[cutIdx]);
			cell[i]->cuts->val[cutIdx] = cell[i]->cuts->val[--cell[i]->cuts->cnt];
			cell[0]->cuts->val[cutIdx]->iStar[i] = cutIdx;
		}
    }
 ...
 ** It is meant to be used for debugging.
 \***********************************************************************/
void print_cut(cutType *cuts, numType *num, int idx) {
    int cnt;

    printf("\nCut #%d:: c:%d o:%d\n  a:%f B:", idx, cuts->val[idx]->cutObs,
           cuts->val[idx]->omegaCnt, cuts->val[idx]->alpha);
