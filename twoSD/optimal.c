...
//		cell->optFlag = FALSE;
//		if ( cell->dualStableFlag ) {
//			/* perform the pre-test */
//			preTest(cell[n]);
//		}
//		cell->optFlag &= cell->optFlag;
//
//
...
//		/* (a) choose good cuts */
//		gCuts[i] = chooseCuts(prob[0], cell[i], cell);
//
...
//
//				/* (c) sample omegas */
//				sampleOmega(cdf[i], observ, cell->k-1);
//
//				/* (d) reform the good cuts by plugging in the
//                 omegas */
//				reform_cuts(cell[i]->sigma, cell[i]->delta, cell[i]->omega, prob[i]->num, prob[i]->coord, gCuts[i], observ, cell->k-1, cell->lbType, prob[0]->lb, prob[0]->num->cols);
//
//				if ( config.MULTI_CUT ) {
//					/* (e1) find out the best reformed cut -> Est at the incumbent solution */
//					Est[i] = gCuts[i]->val[0]->alpha - vXv(gCuts[i]->val[0]->beta, cell->incumbX, NULL, prob[0]->num->cols);
//					for (j = 1; j < gCuts[i]->cnt; j++) {
//						ht = gCuts[i]->val[j]->alpha - vXv(gCuts[i]->val[j]->beta, cell->incumbX, NULL, prob[0]->num->cols);
//						if (Est[i] < ht)
//							Est[i] = ht;
//					}
...
//						Est[j] += cell[i]->weight* (gCuts[i]->val[j]->alpha - vXv(gCuts[i]->val[j]->beta, cell->incumbX, NULL, prob[0]->num->cols));
//				}
//
//				mem_free(observ);
//			}
...
//			AggEst += vXvSparse(cell->incumbX, prob[0]->dBar);
//		else
...
//#endif
//
//		printf("\niter = %d, replication = %d, UB = %f, LB = %f, Gap = %lf", cell->k, m, AggEst, LB, DBL_ABS((AggEst - LB) / cell->incumbEst));
...
//		if (DBL_ABS((AggEst - LB) / cell->incumbEst) <= config.OPTGAP)
//			num_pass++;
//		else {
//			/* Sum up errors of failed replications in full test. Only at Max_iter. */
//			if (cell->k >= config.MAX_ITERATION) {
//				num_failed++;
//				error_sum += (AggEst - LB) / cell->incumbEst;
//			}
//		}
//
...
//			if (cell->k >= config.MAX_ITERATION)
//				cell->full_test_error = error_sum / num_failed;
//			goto TERMINATE;
//		}
