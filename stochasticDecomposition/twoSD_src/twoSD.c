...
char problemType;		// Global variable to store the -t option value
double lowerBoundValue; // Global variable to store the lower bound value
char problemName[256];	// Global variable to store the -p option value
	...
	for (int i = 1; (i < argc); i++)
	{
		if (argv[i][0] == '-')
		{
			...
			case 't':
			{
				...
				{
				case 'l':
					t_value = 'l'; // Assign 'l' to t_value
					config.EPSILON = 0.01;
					config.SCAN_LEN = 128;
					break;
				case 'n':
					t_value = 'n'; // Assign 'n' to t_value
					config.EPSILON = 0.001;
					config.SCAN_LEN = 256;
					break;
				case 't':
					t_value = 't'; // Assign 't' to t_value
...
int readConfig(cString path2config, cString inputDir)
{
	...
	strcpy(line, path2config);
	strcat(line, "config.sd");
	// errMsg("DEBUGBEDUGBEDUG", "DEBUG", line, 0);
	fptr = fopen(line, "r");
	if (fptr == NULL)
	{
