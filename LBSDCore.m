function [stageProbs,bundlePI,bundleJ,OmegaInfo,out] = ...
        LBSDCore(stageProbs,rpm,OmegaInfo,bundlePI,bundleJ,observ,k,constL,opts,hisinfo)
% Stochastic decomposition with level bundle
% Inputs:
...
programMain.varnames{end+1} = 'tau';
...
if numeta >= 1
    ...
    matBeta = stageProbs{t}.obj+bundleJ.beta;
    rhs_add = bundleJ.alpha;
    programMain.obj = [zeros(n1+1,1); 1];
    if bundleJ.cnt > 1
        At_add = [- matBeta(:,1:end-1)', ones(bundleJ.cnt-1,1),zeros(bundleJ.cnt-1,1);
                    - matBeta(:,end)', 0, 1];
        adjust_flow = max(bundleJ.alpha(1:end-1) + matBeta(:,1:end-1)' * hisinfo.x);
    else
        At_add = [- matBeta(:,end)', 0, 1];
        adjust_flow = constL;
        
    end
    programMain.A = [stageProbs{t}.D, zeros(m1,2); zeros(1,n1), 1, 0; zeros(1,n1), 1, -1; At_add];
    f_up = max(bundleJ.alpha + matBeta' * hisinfo.x);
...
while(loopit > 0)
    if numeta >= 1
        % adjust_flow = coef * f_low + (1-coef) * constL
        gap = f_up - adjust_flow;
        val_lev = adjust_flow + opts.lev_coef * gap;
        programMain.rhs = [stageProbs{t}.r; val_lev; 0 ; rhs_add];
        sensecnt = size(At_add,1);
    else
        val_lev = inf;
        programMain.obj = [zeros(n1,1); 0; 1];
        programMain.A = [stageProbs{t}.D, zeros(m1,2); zeros(1,n1), 0, 1; zeros(1,n1), 1, -1];
        programMain.rhs = [stageProbs{t}.r; inf; 0];
    ...
    if strcmp(outxt.status,'OPTIMAL')
        ...
        fprintf("[It%d]not optimal! loopit = %d\n",k,loopit);
        % f_low = hisinfo.val_lev + (loopit-1) * 0.01*abs(hisinfo.val_lev);
        % f_low = max([val_lev,hisinfo.val_lev]);
    end
    loopit = loopit + 1;
end
...
out.lb = lb;
out.x = x{1};
% out.f_low = coef * f_low + (1-coef) * constL;
out.f_low = f_low;
out.val_lev = val_lev;
end
