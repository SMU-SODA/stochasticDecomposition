...
int solveSubprob(probType *prob, oneProblem *subproblem, vector Xvect, vector obsVals, BOOL *spFeasFlag, double *subprobTime, vector piS, double *mubBar) {
	...
	rhs = computeRHS(prob->num, prob->coord, prob->bBar, prob->Cbar, Xvect, obsVals+offset);
	if ( rhs == NULL ) {
	...
	cost = computeCostCoeff(prob->num, prob->coord, prob->dBar, obsVals, offset);
	if ( cost == NULL ) {
